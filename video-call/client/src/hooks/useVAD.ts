// Voice Activity Detection simples via AnalyserNode + RMS threshold.
// Dispara callbacks onSpeakStart / onSpeakStop por peer.
// Polling a cada 100ms — leve o suficiente para não impactar performance.
...
interface VADOptions {
  ...
  threshold?: number;
  /** ms silêncio para considerar parou de falar. Default: 800 */
  silenceMs?: number;
...
export function useVAD(
  ...
  useEffect(() => {
    ...
    // Conecta streams existentes
    streams.forEach((s, id) => attach(id, s));
    if (localStream) attach(localPeerId, localStream);

    // Observa novos streams adicionados externamente via polling no map
    // (o map é passado por referência, então sempre reflete o estado atual)

    const buf = new Uint8Array(256);
    const now = () => Date.now();

    intervalRef.current = setInterval(() => {
      // Adiciona novos streams que surgiram
      streams.forEach((s, id) => attach(id, s));
