...
import { emitDebugEvent } from '../../debug/debug-emitter/index.ts';
import type { ChatMessage } from '../useChat/useChat.types.ts';
import type { LibrasIntegrationOptions } from './useLibrasIntegration.types.ts';
...
export function useLibrasIntegration(
  ...
  useEffect(() => {
    ...
    const newMessages = messages.slice(seenRef.current).filter((m) => !m.isMine);
    ...
    emitDebugEvent('stt', '[system] sent text to glosa converter', { count: texts.length });
